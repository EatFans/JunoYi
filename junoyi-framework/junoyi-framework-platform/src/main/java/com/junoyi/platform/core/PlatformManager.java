...
public class PlatformManager {
    ...
    /**
     * 构造函数 - 自动注册所有 OAuth Provider
     *
     * @param oauthProviders OAuth Provider 列表，由 Spring 自动注入
     */
    public PlatformManager(List<OAuthProvider> oauthProviders) {
     ...
     *
     * @param oauthProviders OAuth Provider 列表
     * @param payProviders   Pay Provider 列表
     */
    public PlatformManager(List<OAuthProvider> oauthProviders, List<PayProvider> payProviders) {
        // 注册 OAuth Provider
        if (oauthProviders != null) {
            for (OAuthProvider provider : oauthProviders) {
                String key = buildOAuthKey(provider.getPlatform(), provider.getCapability());
